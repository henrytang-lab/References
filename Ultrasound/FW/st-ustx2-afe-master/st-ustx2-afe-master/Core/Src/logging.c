...
int _write(int fd, const void *buf, size_t count){
	...
	{
		HAL_StatusTypeDef com_tx_status = HAL_UART_Transmit(&huart5, src, count, 10);
...
static uint8_t usart_start_tx_dma_transfer(void) {
    if (usart_tx_dma_current_len == 0 && (usart_tx_dma_current_len = lwrb_get_linear_block_read_length(&usart_tx_buff)) > 0) {
		...
		if(HAL_UART_Transmit_DMA(&huart5, (uint8_t*)lwrb_get_linear_block_read_address(&usart_tx_buff), usart_tx_dma_current_len)!= HAL_OK)
		{
			Error_Handler();
		}
    }
