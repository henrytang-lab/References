...
class UartPacket:
    def __init__(self, id=None, packet_type=None, command=None, data=None, buffer=None):
        if buffer:
    ...
    def from_buffer(self, buffer: bytes):
        ...
        self.data_len = int.from_bytes(buffer[5:7], 'big')
        self.data = bytearray(buffer[7:7+self.data_len])
        crc_value = util_crc16(buffer[1:7+self.data_len])
        self.crc = int.from_bytes(buffer[7+self.data_len:9+self.data_len], 'big')
...
class UART:
    ...
    def send_ustx(self, id=0, packetType=OW_ACK, command=USTX_NOP, data = None):
        """Send data over UART."""
        if data:
